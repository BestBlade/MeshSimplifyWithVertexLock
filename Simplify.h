...
namespace Simplify
{
	...
	struct Triangle { int v[3]; double err[4]; int deleted, dirty, attr; vec3f n; vec3f uvs[3]; int material; bool locked; };
	...
	std::unordered_map<std::string, std::vector<size_t>> geometries;
		...
		while (fgets(line, 1000, fn) != NULL)
		{
			...
			if (line[0] == 'f')
			{
				...
				if (tri_ok)
				{
					...
					geometries[group].push_back(triangles.size());
	...
	bool lock_vertices(const size_t index)
	{
		if (index >= triangles.size())
			return false;

		triangles[index].locked = true;
		return true;
	}

	bool lock_group(const char* group)
	{
		std::string g = std::string(group);
		if (geometries.find(g) == geometries.end())
			return false;

		for (auto idx : geometries[g])
		{
			if (!lock_vertices(idx))
				return false;
		}
		return true;
	}
